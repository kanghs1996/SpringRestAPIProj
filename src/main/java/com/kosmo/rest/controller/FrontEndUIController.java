...
public class FrontEndUIController {
	...
	public String pushMessage(@RequestParam Map<String, String> message,Model model) throws JsonProcessingException {
		...
		*/	
		//모든 토큰값 가져오기
		List<Map> tokens = dao.selectAllTokens();
		//각 토큰을 갖고 있는 스마트폰으로 푸쉬를 보내기위한 요청(파이어베이스로)
		int success=0;//메시지가 성공적으로 보내진 경우의 숫자 카운팅 저장용
		for(Map map:tokens) {
			for(Object token:map.values()) {
				ResponseEntity<Map> response = requestToFCMServer(message,token);
				System.out.println("상태코드:"+response.getStatusCodeValue());
				System.out.println("응답헤더:"+response.getHeaders());
				ObjectMapper mapper = new ObjectMapper();
				System.out.println("응답바디-JSON:"+mapper.writeValueAsString(response.getBody()));
				System.out.println("응답바디-Map:"+response.getBody());
				
				//ObjectMapper mapper = new ObjectMapper();
				//mapper.readValue(mapper.writeValueAsString(response.getBody()), null)
				Map responseMap=response.getBody();//응답바디
